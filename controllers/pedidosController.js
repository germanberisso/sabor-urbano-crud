...
class PedidosController {
  ...
  async create(req, res) {
    ...
    //   }
    try {
        const { items: itemsBody, tipo, plataforma, nombreCliente, telefonoCliente, direccionCliente } = req.body;
        ...
        for (const id in itemsBody) {
            ...
            if (itemsBody[id].seleccionado) {
              ...
              items.push({ id: producto.id, producto: producto.nombre, precio, cantidad });
        ...
        res.status(201).json({ success: true, message: 'Pedido creado exitosamente', data: nuevoPedido });
      } catch (error) {
        console.error('Error al crear pedido:', error);
        res.status(500).json({ success: false, message: 'Error al crear pedido', error: error.message });
      }
  ...
  async update(req, res) {
    try {
        ...
        if (!pedidoExistente) return res.status(404).json({ success: false, message: 'Pedido no encontrado' });
        ...
        const datosActualizados = { items, itemsText, total, tipo, plataforma, cliente };
    
        const pedidoActualizado = await this.pedidoModel.update(id, datosActualizados);
    
        res.json({ success: true, message: 'Pedido actualizado exitosamente', data: pedidoActualizado });
      } catch (error) {
