...
class Pedido {
  ...
  _validatePedido(nuevoPedido) {
    ...
    if (!nuevoPedido.nombreCliente) {
      nuevoPedido.nombreCliente = null;
      nuevoPedido.telefonoCliente = null;
      nuevoPedido.direccionCliente = null;
    }

    if (nuevoPedido.tipo === "presencial") {
      // Si es tipo mesa
      if (nuevoPedido.numeroOrden?.startsWith("MESA")) {
        nuevoPedido.nombreCliente = null; // Mesa no tiene cliente
      } else {
        // Mostrador -> cliente opcional
        if (
          nuevoPedido.nombreCliente &&
          typeof nuevoPedido.nombreCliente !== "object"
        ) {
    ...
    if (nuevoPedido.tipo === "delivery") {
      ...
      if (
        !nuevoPedido.nombreCliente ||
        !nuevoPedido.telefonoCliente ||
        !nuevoPedido.direccionCliente
      ) {
        throw new Error(
          "Los pedidos de delivery requieren nombreCliente, teléfonoCliente y direccionCliente"
        );
      }
    }
  ...
  async create(nuevoPedido) {
    ...
    const pedido = {
      id: nuevoId,
      numeroOrden:
        nuevoPedido.numeroOrden || `ORD-${nuevoId.toString().padStart(3, "0")}`, // Número de orden automático si no viene
